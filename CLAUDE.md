...
| Daemon (per-thesis) | `./zig-out/bin/praescientia-orchestrate-daemon --kb-root=PATH --per-thesis-cadence [--interval=DUR] [--max-ticks=N] [--screener-cadence=DUR] [--screener-bin=PATH] [--sources-floor=N] [--sources-curator-bin=PATH] [--sources-cache-ttl=DUR] [--sources-thesis-cap=DUR] [--allow-global-sources]` | `tools/orchestrate_daemon.zig` (opt-in via flag) — `--screener-cadence` spawns `praescientia-screener tick` every DUR (min 60s); state persists to `kb/.ticks/.last_screener_scan.ms`. `--sources-floor=N` (per-thesis cadence only) grounds any due thesis with fewer than N `source:<tier>` commentary entries by spawning `praescientia-curator tick` before the analyst dispatch; per-thesis cache state persists to `kb/.ticks/<thesis_id>.last_curator_run.ms` |
